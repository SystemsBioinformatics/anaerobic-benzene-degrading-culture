...
################################################################################################
dir<-".../anaerobic-benzene-degrading-culture/bioreactor/Data"
...
levels(data$accepted) <- c("Bad", "Good")
...
q1<-ggplot(dataf, aes(x=liguid_rna_norm_log, y=biofilm_rna_norm_log,color=accepted)) +
  ...
  geom_text_repel(size=5,parse  = TRUE,box.padding = 0.4,data=subset(dataf, ((liguid_rna_norm_log > 9)&(biofilm_rna_norm_log > 9))),aes(label=bins))+
  geom_segment(aes(x = 1, y = 1, xend = 18, yend = 18),color="red")+
  labs(size="Active Genes %",x="Specific RNA Abundance - Liquid",y="Specific RNA Abundance - Biofilm")+ theme(text = element_text(size=size_all),plot.title = element_text(size=size_all),
axis.title.x = element_text(size=size_all),
axis.title.y = element_text( size=size_all),
legend.title = element_text(size=size_all),
...
output$bins<-substr(x =output$bins, 1,7)
output<-output[,-2]
path_orfn<-paste(dir,"/Bins_orfs_DNA",sep = "")
...
umap<-ggplot(df_big,aes(x=V1,y=V2))+scale_colour_manual(name="Clusters", values= colors_same)+scale_shape_manual(values=c(19,18))+
  geom_point(aes(color=group_dna,shape=Best),size=4,stroke=2)+stat_ellipse(aes(x=V1, y=V2,lty=Groups, group=Groups),type = "norm")+scale_linetype_manual(values=c(3,2,1))+
  ...
  geom_text_repel(size=4,parse  = TRUE,aes(label=names,color=group_dna),point.padding = 0.3)+ theme(text = element_text(size=size_all),
                                                                                                    plot.title = element_text(size=size_all),
                                                                                                    axis.title.x = element_text(size=size_all),
                                                                                                    axis.title.y = element_text( size=size_all), 
                                                                                                    legend.title = element_text(size=size_all),
...
prow2<-plot_grid(legend1,prow, legend2, rel_widths = c(.3,2, .5), ncol = 3, nrow = 1)

# save_plot("one_two_update.pdf",prow2, base_width=16, base_height=5, dpi=300)
# plot_grid(prow )
# 
# plot_grid(prow)
# ggsave("one_two_update2.pdf",prow2, units="in", width=15, height=5, dpi=300)
plot_grid(prow2)
# dev.off()
```
